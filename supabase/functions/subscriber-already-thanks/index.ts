// supabase/functions/subscriber-already-thanks/index.ts
//
// Called when someone tries to subscribe with an email that's already
// on the list. Instead of a "you're subscribed!" welcome email, sends
// a short thank-you noting how long they've been subscribed, with a
// preview of the 3 most recent posts.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const since = subscriber?.created_at
      ? new Date(subscriber.created_at).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })
      : null
    ...
    const html = `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; color: #0F1B33;">
        <h2 style="font-family: Georgia, serif;">Thanks for sticking around!</h2>
        <p>You're already subscribed to the True Doc Pros blog${since ? ` — since ${since}` : ''}. We appreciate
        you being here.</p>
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: fromEmail,
        to: email,
        subject: 'Thanks for being a True Doc Pros blog subscriber',
        html,
      }),
    })

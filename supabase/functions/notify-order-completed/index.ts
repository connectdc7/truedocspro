// supabase/functions/notify-order-completed/index.ts
//
// Called from the staff order page when the "Order completed" toggle
// is switched on. Sends the client a final confirmation email.
  ...
  try {
    ...
    const html = `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; color: #0F1B33;">
        <h2 style="font-family: Georgia, serif;">Your order is complete</h2>
        <p>Hi ${firstName},</p>
        <p><strong>${order.document_name}</strong> is fully complete — every step is finished, and if it was
        mailed back to you, it's on its way.</p>
        <p>Thank you for trusting True Doc Pros with your document.</p>
        <p style="margin-top: 24px;">
          <a href="${siteUrl}/portal/orders/${order_id}"
             style="background:#3F6B4A;color:#fff;padding:10px 20px;border-radius:999px;text-decoration:none;">
            View your order
          </a>
        </p>
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: fromEmail,
        to: clientEmail,
        subject: `Your order is complete — ${order.document_name}`,
        html,
      }),
    })
